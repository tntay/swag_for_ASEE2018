...
class BLECentralViewController : UIViewController, CBCentralManagerDelegate, CBPeripheralDelegate, UITableViewDelegate, UITableViewDataSource{
    ...
    var temperature_data : Int = 0    // Sensor readings
    var humidity_data : Int = 0       //
    var pressure_data : Int = 0       //
    ...
    /*
     *  Refresh button action function
     */
    ...
    func startScan() {
        ...
        // Modified withServices: nil --- Original withServices: [BLEService_UUID]
        //centralManager?.retrievePeripherals(withIdentifiers: [the_id!])
    ...
    func peripheral(_ peripheral: CBPeripheral, didDiscoverCharacteristicsFor service: CBService, error: Error?) {
        ...
        for characteristic in characteristics {
            ...
            //look for the 3 characteristic were interested in: temperature, humidity, pressure
            
            if characteristic.uuid.isEqual(BLE_Characteristic_uuid_temperature)  {
                ...
                //print("Temperature Characteristic: \(characteristic.uuid)")
            }
            if characteristic.uuid.isEqual(BLE_Characteristic_uuid_humidity){
                humidityCharacteristic = characteristic
                
                //Once found, subscribe to the this particular characteristic...
                peripheral.setNotifyValue(true, for: humidityCharacteristic!)
                peripheral.readValue(for: characteristic)
                //print("Humidity Characteristic: \(characteristic.uuid)")
            }
            if characteristic.uuid.isEqual(BLE_Characteristic_uuid_pressure){
                pressureCharacteristic = characteristic
                
                //Once found, subscribe to the this particular characteristic...
                peripheral.setNotifyValue(true, for: pressureCharacteristic!)
                peripheral.readValue(for: characteristic)
                //print("Pressure Characteristic: \(characteristic.uuid)")
            }
    ...
    func peripheral(_ peripheral: CBPeripheral, didUpdateValueFor characteristic: CBCharacteristic, error: Error?) {
        ...
        if characteristic == temperatureCharacteristic {
            ...
            // Store the bytes recieved over BLE into standard datatype
            var temp : Int = 0
            let nsData = characteristic.value! as NSData
            let rawPtr = nsData.bytes
            let data = NSData(bytes: rawPtr, length:4)
            data.getBytes(&temp, length:4)
            temperature_data = temp
            //print("Temerature: \((temperature_data))")
            
            // Post new sensor data to NC (both string value and integer value)
            let string_data = String(temperature_data)
            let dataToSend = [string_data: temperature_data]
            NotificationCenter.default.post(name: .notifyName_temp, object: nil, userInfo: dataToSend)
        }
        if characteristic == humidityCharacteristic {
            ...
            let nsData = characteristic.value! as NSData
            let rawPtr = nsData.bytes
            let data = NSData(bytes:rawPtr, length:1)
            data.getBytes(&temp, length:1)
            humidity_data = temp
            //print("Humidity: \((humidity_data))")
            
            // Post new sensor data to NC (both string value and integer value)
            let string_data = String(humidity_data)
            let dataToSend = [string_data: humidity_data]
            NotificationCenter.default.post(name: .notifyName_hum, object: nil, userInfo: dataToSend)
        }
        if characteristic == pressureCharacteristic {
            ...
            // Post new sensor data to NC (both string value and integer value)
            let string_data = String(pressure_data)
            let dataToSend = [string_data: pressure_data]
            NotificationCenter.default.post(name: .notifyName_pres, object: nil, userInfo: dataToSend)
        }
    }
    ...
    func peripheral(_ peripheral: CBPeripheral, didDiscoverDescriptorsFor characteristic: CBCharacteristic, error: Error?) {
        ...
        if ((characteristic.descriptors) != nil) {
            ...
            for x in characteristic.descriptors!{
                ...
                //print("Temperature Value \(String(describing: temperatureCharacteristic?.value))")
                //print("Humidity Value \(String(describing: humidityCharacteristic?.value))")
                //print("Pressure Value \(String(describing: pressureCharacteristic?.value))")
            }
        }
    }
    ...
    func centralManager(_ central: CBCentralManager, didDisconnectPeripheral peripheral: CBPeripheral, error: Error?) {
        ...
        print("Disconnected")
        NotificationCenter.default.post(name: .notifyName_BleDisconnect, object: nil)
    }
